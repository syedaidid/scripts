...
# Variable to store previous metadata
prev_metadata=""
...
while true; do
  ...
  if [ "$metadata" != "$prev_metadata" ]; then
    ...
    # Variables
    # ====================================================================
    # dunst title - truncate length to 40 characters with awk
    ...
    # if urlcheck not equal to 0 it's an http link
    if [ "${iconcheck}" -ne 0 ]; then
      ...
      if [ -z "${arturl}" ]; then
        ...
        icon="${imagetmp}" # set icon to ffmpeg thumbnail
      else
        imagetmp=$(mktemp /tmp/playerctlXXX --suffix=.jpg)
    ...
    if [ "${urlcheck}" -eq 7 ]; then
      ...
      icon="${output}" # set icon to ffmpeg thumbnail
    fi
